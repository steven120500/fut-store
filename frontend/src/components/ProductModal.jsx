...
export default function ProductModal({
  ...
  const [editedIsNew, setEditedIsNew] = useState(product?.isNew || false); // ✅ NUEVO
  ...
  useEffect(() => {
    ...
    setEditedIsNew(product?.isNew || false); // ✅ sincroniza
    setLocalImages(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEditedIsNew(updated?.isNew || false);
      setLocalImages(
      ...
      >
        ...
        <div className="mt-4">
          ...
          {tallasVisibles.map((talla) => (
            <div
              key={talla}
              className="flex justify-between items-center border rounded px-2 py-1 mb-1 text-[#d4af37]"
            >
              <span>{talla}</span>
              {isEditing ? (
                <input
